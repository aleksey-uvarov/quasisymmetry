...
if __name__=="__main__":
    ...
    parser.add_argument("--initialguess")
    ...
    out_row = [args.mol, args.bond, E_fci, variance_before, variance_after,
                               f(x_id), f(res.x), 1, b_opt / a_opt, c_opt / a_opt]

    np.savetxt(args.mol + "_" + str(args.bond) + "_comm_opt.txt", res.x)
    ...
    if csv_file.is_file():
        with open(csv_filename, "a", newline="") as f:
            ...
            # writer.writeheader()
            writer.writerow({fieldnames[j]: out_row[j] for j in range(len(fieldnames))})
    else:
